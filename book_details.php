...
if ($book_id > 0) {
    // Insert a view record into the views table to count the view
    $view_sql = "INSERT INTO views (book_id) VALUES ($book_id)";
    if ($conn->query($view_sql) === TRUE) {
        // View inserted successfully, now fetch the book details
    } else {
        echo "Error recording view: " . $conn->error;
    }
...
<head>
    ...
    <style>
        ...
        .next-books-list li {
            flex: 1 1 calc(20% - 20px); /* Default layout for larger screens */
                    ...
                    <?php foreach ($nextBooks as $nextBook): ?>
                        <li>
                            <a href="book_details.php?id=<?php echo $nextBook['id']; ?>">
                                ...
                                <span><?php echo htmlspecialchars($nextBook['title']); ?></span>
                            </a>
                        </li>
                    <?php endforeach; ?>
                </ul>
            <?php else: ?>
                <p>No next books available.</p>
            <?php endif; ?>
        </div>
    </div>
</body>
